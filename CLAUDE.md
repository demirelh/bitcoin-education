...
Flux image downloads retry transient CDN failures before imagegen records a
failed chapter entry. If a chapter picture still cannot be produced, the
profile's `fallback_provider` is tried, and only then does the stage fail —
imagegen never reports success with a manifest entry pointing at a file that
was never written.

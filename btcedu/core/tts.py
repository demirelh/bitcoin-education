...
#: Where every take is levelled to. ElevenLabs returns the same voice up to
#: 10 dB apart from one generation to the next, which is plainly audible when
#: two segments follow each other. The figure sits just under YouTube's own
...
def _normalize_loudness(
    ...
    *,
    ...
    take. The delivery is therefore left exactly as the engine produced it and
    only its level is corrected — a one-pass filter would ride the gain within
    the sentence and audibly pump.

    The peak ceiling is what stops a very quiet take from clipping when it is
    lifted; only there does the filter's limiter engage at all.

    Returns None when nothing was changed. Levelling is an improvement, never
    a precondition — every failure leaves the original file untouched.
    """
    import subprocess
...
def _parse_loudnorm_json(stderr: str) -> dict[str, str] | None:
    ...
    simply means the take is left at the level it came in at.
    """
    start = stderr.rfind("{")
    end = stderr.rfind("}")
    ...
    if not all(key in stats for key in required):
        return None
    # A digitally silent take measures as -inf and cannot be levelled.
